        ...
        <div class="w-full max-w-2xl px-6 py-4 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
            ...
            <form wire:submit.prevent='save'>
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    <div>
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('messages.form_name') }}</label>
                    ...
                    <div>
                        <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('messages.form_email') }}</label>
                ...
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300" for="file_input">{{ __('messages.form_file_upload') }}</label>
                ...
                <div class="lg:flex justify-between gap-2 mt-6">
                    <div class="flex flex-row">
                        <div class="flex items-center">
                            <p class="text-sm lg:text-base font-bold text-gray-900 dark:text-gray-500">{{ __('messages.form_review') }}:</p>
                        </div>   
                        <div class="flex items-center">
                            <label for="star1">
                                <input hidden wire:model="rating" class="hidden" type="radio" id="star1" name="rating" value="1"/>
                                <svg aria-hidden="true" class="w-5 h-5 cursor-pointer @if($rating >= 1) text-yellow-400 @else text-gray-300 dark:text-gray-500 hover:text-yellow-400 @endif" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>{{ __('messages.form_review_star1') }}</title><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                            </label>
                            <label for="star2">
                                <input hidden wire:model="rating" class="hidden" type="radio" id="star2" name="rating" value="2"/>
                                <svg aria-hidden="true" class="w-5 h-5 cursor-pointer @if($rating >= 2) text-yellow-400 @else text-gray-300 dark:text-gray-500 hover:text-yellow-400 @endif" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>{{ __('messages.form_review_star2') }}</title><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                            </label>
                            <label for="star3">
                                <input hidden wire:model="rating" class="hidden" type="radio" id="star3" name="rating" value="3" />
                                <svg aria-hidden="true" class="w-5 h-5 cursor-pointer @if($rating >= 3) text-yellow-400 @else text-gray-300 dark:text-gray-500 hover:text-yellow-400 @endif" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>{{ __('messages.form_review_star3') }}</title><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                            </label>
                            <label for="star4">
                                <input hidden wire:model="rating" class="hidden" type="radio" id="star4" name="rating" value="4" />
                                <svg aria-hidden="true" class="w-5 h-5 cursor-pointer @if($rating >= 4) text-yellow-400 @else text-gray-300 dark:text-gray-500 hover:text-yellow-400 @endif" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>{{ __('messages.form_review_star4') }}</title><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                            </label>
                            <label for="star5">
                                <input hidden wire:model="rating" class="hidden" type="radio" id="star5" name="rating" value="5" />
                                <svg aria-hidden="true" class="w-5 h-5 cursor-pointer @if($rating >= 5) text-yellow-400 @else text-gray-300 dark:text-gray-500 hover:text-yellow-400 @endif" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>{{ __('messages.form_review_star5') }}</title><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                            </label>
                        </div>
                    </div>

                    <button type="submit" class="px-15 py-2 mt-4 lg:mt-0 w-full lg:w-52 text-gray-900 dark:text-gray-400 font-bold transition-colors duration-200 transform bg-yellow-400 rounded-md hover:bg-yellow-300 focus:outline-none focus:bg-gray-600">{{ __('messages.form_submit_button') }}</button>
                </div>
            </form>
